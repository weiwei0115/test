(() => {
  ...
  function updateHUD(){
    ...
    undoBtn.disabled = history.length <= 1 || aiBusy || scoringMode;
    passBtn.disabled = aiBusy || scoringMode;
    newBtn.disabled = aiBusy || scoringMode;
    resignBtn.disabled = aiBusy || scoringMode;
  }
  ...
  function geom(){
    ...
    const W = rect.width; // CSS px（邏輯尺寸）
  ...
  // ===== Canvas 尺寸：只在初始化/resize 時設定（修掉 hover 放大/抖動） =====
  function resizeCanvas(){
    ...
    // 之後所有 draw 都用 CSS px 畫（transform 把單位轉成 CSS px）
  ...
  // ===== 棋盤繪製（核心） =====
  function draw(){
    const { W, pad, g, r } = geom();

    // 清畫面（注意：ctx 已經 setTransform(DPR,...) 所以用 CSS px 清）
  ...
  function drawLiveAreaOverlay(now){
    ...
    if (typeof ctx.roundRect === "function") {
      ...
      // fallback：簡單矩形
      ctx.rect(cx - r*0.8, cy - r*1.25, r*1.6, r*0.55);
    }
    ctx.fill();
  ...
  function maybeAIMove(){
    ...
    setTimeout(() => {
      try{
        ...
        aiBusy = false;
        aiThinking.textContent = scoringMode ? "點目模式" : "";
        updateHUD();
      }
    }, 30);
  }
  ...
  scoreBtn.addEventListener("click", () => enterScoring());
  exitScoreBtn.addEventListener("click", () => exitScoring());
